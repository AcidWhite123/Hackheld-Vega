...
#define FRAME_CONTROL_TSQUEST while((currentMillis-MemMillis)<36){currentMillis=millis();}MemMillis=currentMillis
#define FRAME_CONTROL_2_TSQUEST while((currentMillis-MemMillis)<60){currentMillis=millis();}MemMillis=currentMillis
...
display.buffer[(x+(y*128))]=(SPEED_BLITZ(24, 2, x, y, 0, INTRO_TSQUEST) | pgm_read_byte(&BACKGROUND_TSQUEST[x + ((y)*128)]) | Blink_Start_TSQUEST(x, y, BL_));
    }
...
void JoyPad_Refresh_TSQUEST(void) {
  ...
  if (TINYJOYPAD_LEFT==0) { Sub_Main.MOVE_X_L(); }
  if (TINYJOYPAD_RIGHT==0) { Sub_Main.MOVE_X_R(pgm_read_byte(&T_SUBMAIN_TSQUEST[0])); }
  if (TINYJOYPAD_UP==0) { Sub_Main.MOVE_Y_U(); }
  if (TINYJOYPAD_DOWN==0) { Sub_Main.MOVE_Y_D(6); }
  if (BUTTON_DOWN) {
...
void CHECK_COLLISION_BALLISTIC_TSQUEST(void) {
  RECUP_RANGE_TSQUEST(RecupeLineY_TSQUEST(Sub_Main.GET_BallisticY()));
...
void CHECK_OTHER_COLLISION_BALLISTIC_TSQUEST(void) {
  RECUP_RANGE_TSQUEST(RecupeLineY_TSQUEST(Sub_Main.GET_Y()));
  for (uint8_t t = SQUEST.sa; t < SQUEST.eb; t++) {
...
void UPDATE_BALLISTIC_TSQUEST(void) {
  ...
  for (uint8_t t = 0; t < 9; t++) {
    ...
    if (Other_Sprite[t].GET_A() == 3) {
      if ((Other_Sprite[t].GET_Ballistic() == 0) && (One_Shoot == 0) && (Check_if_Deployed_TSQUEST(t) == 0)) { Other_Sprite[t].BallisticDeploy(pgm_read_byte(&SUB2_TSQUEST[0]), SQUEST.Speed_Balistic); }
    }
...
uint8_t RECUPE_OTHER_TSQUEST(uint8_t xPASS, uint8_t yPASS) {
  ...
  for (uint8_t t = SQUEST.sa; t < SQUEST.eb; t++) {
    if (Other_Sprite[t].GET_A() != 0) {
      ...
      BYTE_SUM |= blitzSprite(Other_Sprite[t].GET_X(), Other_Sprite[t].GET_Y() + Recup_sin_Add_TSQUEST(Other_Sprite[t].GET_A()), xPASS, yPASS, (Other_Sprite[t].GET_D() + SQUEST.Frame), SpR[(Other_Sprite[t].GET_A()) - 1]);
NeXt:;
    }
  }
  return BYTE_SUM;
...
void RENDER_TSQUEST(void) {
  ...
  for (y = 1; y < 7; y++) {
    ...
    for (x = 0; x < 128; x++) {
   display.buffer[(x+(y*128))]=(
      Recupe_SUBsolo_TSQUEST(x, y) | RecupeRanged_TSQUEST(x, y) | RECUPE_MAIN_TSQUEST(x, y) | pgm_read_byte(&BACKGROUND_TSQUEST[x + ((y)*128)]));
   }
 }
   for (x = 0; x < 128; x++) {
   display.buffer[(x+(y*128))]=(FullDisplayRefresh_TSQUEST(FlipFlop1, FlipFlop2, x, 7) & pgm_read_byte(&BACKGROUND_TSQUEST[x + ((7)*128)]));
  }
display.display(); 
}
